...
async def get_tasks(
    ...
    try:
        tasks_for_cache = [TaskRead.model_validate(t).model_dump(mode="json") for t in tasks]
...
async def update_task(
    ...
    if task_data.status:
        if task.status == TaskStatus.DONE and task_data.status != TaskStatus.DONE:
            logger.info(f"Update rejected: Attempt to revert DONE status on ID {task_id}")
            raise HTTPException(status_code=400, detail="Нельзя изменить статус из 'done'")
